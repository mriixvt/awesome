...
vicious.register(pacwidget, vicious.widgets.pkg,
  function(widget, args)
    ...
    return args[1]
  end, 1801, "Arch") -- Arch S for ignorepkg
...
mytextclock = awful.widget.textclock(markup("#7788af", "%a %d ") .. markup("#343639", ">") .. markup("#de5e1e", " %H:%M:%S"))
...
fswidget = lain.widgets.fs({
    settings  = function()
        widget:set_markup(markup("#80d9d8", fs_now.used .. "% [S]"))
    end
})
...
fs3widget = lain.widgets.fs3({
    settings  = function()
        widget:set_markup(markup("#80d9d8", fs_now.used .. "% [W]"))
    end
})
...
volumewidget = lain.widgets.alsa({
    settings = function()
        ...
        widget:set_markup(markup("#33FFFF", "▲♬ " .. volume_now.level .. "% "))
    end
})
...
function popup_mpd()
	...
	mpd_notif = current
	mpd_notift = artist .. "\n" .. title
	naughty.notify { icon = mpd_notif, text = mpd_notification_preset.text }
end
mpdwidget:buttons(awful.util.table.join(awful.button({ }, 1, popup_mpd)))
